...
\addlinespace
Fluxograma de comercialização & Destino da produção, elos intermediários e margens por canal & §4.6--4.7; §6.3 & Elos não monetários de circulação e remuneração em espécie registrados em §9.8 \\
\addlinespace
Diagrama de Venn institucional & Rede de organizações atuantes, proximidade percebida e influência sobre o manejo & §5.9; §6.1 & Entidades concorrentes de representação, que originaram os itens de governança em §9.2 \\
\addlinespace
Matriz FOFA comunitária & Forças, fragilidades, oportunidades e ameaças atribuídas à tecnologia & §6.8--6.9 & Ameaças de natureza fundiária e documental alocadas em §9.5 e §9.6 \\
\addlinespace
Matriz de priorização por pares & Ordenação de impactos e urgências de salvaguarda em escores de Borda normalizados & §6.1--6.7 & Divergência entre estratos de gênero e geração, que motivou a especificação de desagregação da resposta \\
\bottomrule
\end{tabular}
...
A correspondência entre o instrumento original em inglês \cite{Liniger2019} e a versão sintetizada foi conferida antes do encaminhamento ao comitê. Os quarenta e oito itens das seções 1 a 7 foram pareados sem omissão nem acréscimo. O comitê avaliou a equivalência semântica de cada item.
...
Foram aceitos os itens que alcançaram $IVC \geq 0{,}80$, retornaram para revisão os situados no intervalo $0{,}60 \leq IVC < 0{,}80$ e foram reformulados ou excluídos aqueles com $IVC < 0{,}60$. A concordância interavaliadores foi aferida por kappa de Fleiss \cite{Fleiss1971}. O instrumento de avaliação foi construído e reuniu os quarenta e oito itens das seções 1 a 7 da versão traduzida, dispostos por seção, com uma coluna por dimensão de equivalência, campo próprio para a redação alternativa proposta pelo avaliador e uma segunda via contendo o texto integral de cada item para consulta durante o julgamento. O comitê também identificou lacunas culturais e propôs itens suplementares.

\subsubsection{Etapa 5, Pré-Teste}

A versão pré-final foi aplicada a 10--15 agricultores quilombolas de Jeremoabo (BA), selecionados por amostragem intencional com variabilidade em idade, gênero, escolaridade e sistema produtivo. A aplicação ocorreu em formato de entrevista assistida. Ao término de cada seção, um debriefing cognitivo \cite{Willis2005} examinou a compreensão do enunciado, a adequação das alternativas linguísticas e a pertinência experiencial do item.
...
A validação do instrumento WOCAT-SLM-QBR seguiu o modelo tripartite descrito no \textit{Standards for Educational and Psychological Testing} \cite{Association2018}, adaptado ao contexto de instrumentos de avaliação socioecológica.
...
As análises de validade de conteúdo e de concordância foram conduzidas em R versão 4.5.1 \cite{RCore2024}, empregando o pacote \texttt{irr} para o kappa de Fleiss e o coeficiente $W$ de Kendall e rotina própria para o cômputo do IVC conforme o algoritmo de \citet{Hernandez-Nieto2002}. Os resultados determinaram quais itens seguiram para as etapas subsequentes de modelagem fuzzy.
...
A série de entrevistas semiestruturadas abertas iniciada na etapa formativa prosseguiu por amostragem teórica com agricultores quilombolas de Jeremoabo. O roteiro incorporou a percepção sobre as variáveis levadas ao Delphi, eventuais dimensões não contempladas, a adequação da linguagem e a hierarquização espontânea de urgências, sem abandonar o formato aberto. A saturação teórica das categorias centrais determinou o encerramento \cite{Glaser1967}, com oito a dez entrevistas adicionais conduzidas conforme a densidade das categorias já constituídas.

As entrevistas foram gravadas em áudio, transcritas integralmente, conferidas quanto à atribuição de turnos e ao vocabulário vernacular e submetidas aos ciclos de codificação adotados na etapa formativa \cite{CorbinStrauss2015,Saldana2021}. Dois pesquisadores conduziram a codificação de forma independente, com kappa de Cohen $\geq 0{,}60$ \cite{Landis1977}. Uma matriz relacionou as variáveis validadas pelo painel às categorias axiais emergentes. A análise temática \cite{Braun2006} foi utilizada apenas na sumarização descritiva.
...
O mapeamento examinou possíveis lacunas nas dimensões espiritual-ritual, de transmissão intergeracional por oralidade e de coletividade associada a bens comuns. Somente as lacunas sustentadas pelos dados foram convertidas em itens suplementares nas dimensões $V_1$--$V_8$ pertinentes. A análise testou se categorias \textit{etic} deslocam referentes locais quando aplicadas a ontologias distintas \cite{Herdman1999}, problema relacionado à vulnerabilidade documental dos sistemas de conhecimento indígena \cite{Maina2012,Balogun2021}.
...
Uma vez validado o instrumento, suas variáveis alimentaram as funções de pertinência e as regras de inferência de um sistema difuso do tipo Mamdani. A origem de cada variável do ISB foi documentada para permitir o rastreamento entre WOCAT original $\rightarrow$ WOCAT-SLM-QBR $\rightarrow$ consenso Delphi $\rightarrow$ meta-análise $\rightarrow$ modelo fuzzy \cite{Costanza1997}. O manual registrou as decisões do protocolo integrado de adaptação e Delphi.

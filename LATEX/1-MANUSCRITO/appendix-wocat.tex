...
%
% Fonte: tese-catuxe/DADOS/questionario/2-adaptado_WOCAT-SLM-QBR/
%        conteudo_WOCAT-SLM-QBR.tex
% Gerado por: tese-catuxe/tools/propaga_questionario.py
%
% A cópia existe porque o manuscrito de submissão precisa ser
% autocontido. Qualquer correção deve ser feita na fonte.
% ============================================================

\appendix
\section*{Apêndice~A --- Questionário WOCAT-SLM-QBR sobre Tecnologias de Manejo Sustentável da Terra}
\addcontentsline{toc}{section}{Apêndice~A --- Questionário WOCAT-SLM-QBR}
\label{app:wocat}
\setcounter{table}{0}
...
\noindent\textbf{Fonte:} WOCAT --- \textit{World Overview of Conservation Approaches and Technologies}, Centre for Development and Environment (CDE), Universidade de Berna, Suíça. Versão 2019 (WOCAT, 2019).\\[6pt]
...
\medskip\noindent\textit{Condições:} Os dados capturados são armazenados no banco de dados online do WOCAT. Os dados são de acesso aberto e disponibilizados sob a licença \textit{Creative Commons Attribution-NonCommercial-ShareAlike 3.0 Unported}.
...
%% =========================================================
\subsection*{9\quad Seção Suplementar --- Itens para Avaliação de Vulnerabilidade Biocultural ($V_1$--$V_8$)}
\label{sec:suplementar}

\noindent\textit{Esta seção contém itens suplementares não cobertos pelo questionário WOCAT original, desenvolvidos para operacionalizar as oito dimensões de vulnerabilidade biocultural ($V_1$--$V_8$) em contexto de comunidades quilombolas. As Camadas 1 a 3 concentram as lacunas que o instrumento original não cobre com densidade suficiente, sobretudo em $V_1$--$V_5$. A Camada 4 agrega módulos focais complementares para $V_6$--$V_8$, cuja captura permanece apoiada nas seções originais do instrumento e em regras de codificação e mediação de aplicação em campo.}
...
%% --- V5 ---
\medskip\noindent\textbf{9.1.6\quad Idade de entrada no trabalho agrícola e escolarização}\\[2pt]
Em que idade as crianças da comunidade passaram a acompanhar o trabalho na roça, na geração do respondente e na geração atual?\\[2pt]
Geração do respondente: \dotfill\ anos \qquad Geração atual: \dotfill\ anos\\[2pt]
Qual o principal motivo da diferença, se houver?\\
$\square$~Escolarização prolongada \quad $\square$~Renda previdenciária ou assistencial na família \quad $\square$~Migração \quad $\square$~Redução da área cultivada \quad $\square$~Outro: \dotfill

\medskip\noindent\textbf{9.1.7\quad Destino ocupacional dos descendentes}\\[2pt]
Dos filhos e netos dos atuais praticantes, qual a proporção que permanece na atividade agrícola no território?\\
$\bigcirc$~Nenhum \quad $\bigcirc$~Poucos ($<$\,25\%) \quad $\bigcirc$~Alguns (25--50\%) \quad $\bigcirc$~A maioria (50--75\%) \quad $\bigcirc$~Quase todos ($>$\,75\%)\\[2pt]
Ocupações e destinos predominantes dos que saíram: \dotfill\\[2pt]
Houve retorno de algum descendente ao território? \quad $\bigcirc$~Não \quad $\bigcirc$~Sim\\[2pt]
Se sim, motivo declarado do retorno: \dotfill

\medskip\noindent\textbf{9.1.8\quad Permanência dos aprendizes e continuidade da transmissão}\\[2pt]
A transmissão desta prática depende da permanência do aprendiz no território ao longo do ciclo produtivo?\\
$\bigcirc$~Não, pode ser aprendida em visitas esporádicas \quad $\bigcirc$~Em parte \quad $\bigcirc$~Sim, exige acompanhamento do ciclo completo\\[2pt]
Etapas da prática que só se aprendem presencialmente e na estação própria: \dotfill

\medskip\noindent\textbf{9.1.9\quad Acoplamento entre calendário festivo e ciclo produtivo}\\[2pt]
Existem festas, novenas ou celebrações cuja data se vincula a etapas do ciclo produtivo (plantio, colheita, início das chuvas)?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim\\[2pt]
Se sim, relacione a celebração, o mês e a etapa produtiva correspondente:\\[2pt]
\dotfill\\[2pt]\dotfill\\[2pt]
Alguma dessas celebrações cumpre função de petição ou agradecimento relativo à chuva?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim. Especifique: \dotfill

\medskip\noindent\textbf{9.1.10\quad Cronologia da ocupação por gerações nomeadas}\\[2pt]
A comunidade situa sua fundação e os marcos de introdução e abandono de práticas por referência a pessoas ou gerações nomeadas?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim\\[2pt]
Se sim, registre a sequência de gerações desde a fundação, com o nome pelo qual cada uma é referida localmente e, quando houver, a data aproximada:\\[2pt]
\dotfill\\[2pt]\dotfill\\[2pt]\dotfill\\[2pt]
Existe marco material associado à geração fundadora (casa, cemitério, poço, árvore)?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim. Especifique e informe o estado de conservação: \dotfill

\medskip\noindent\textbf{9.1.11\quad Rede de parentesco entre povoados}\\[2pt]
Há vínculos genealógicos que atravessam os limites desta comunidade e alcançam povoados vizinhos?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim\\[2pt]
Se sim, indique os povoados alcançados e o que circula por essa rede:\\
$\square$~Casamento \quad $\square$~Sucessão de áreas \quad $\square$~Troca de sementes \quad $\square$~Mão de obra em mutirão \quad $\square$~Outro: \dotfill\\[2pt]
Povoados: \dotfill
...
%% ---------------------------------------------------------
\subsubsection*\medskip\noindent\textbf{9.2.5\quad Entidades que reivindicam representação}\\[2pt]
Quantas entidades reivindicam representar esta comunidade perante órgãos públicos?\\
$\bigcirc$~Nenhuma \quad $\bigcirc$~Uma \quad $\bigcirc$~Duas \quad $\bigcirc$~Três ou mais\\[2pt]
Relacione cada entidade, o ano de fundação e o segmento que declara representar:\\[2pt]
\dotfill\\[2pt]\dotfill\\[2pt]
\textit{Nota de aplicação: quando houver mais de uma entidade, a anuência para a documentação deve ser obtida de todas, e o registro deve indicar qual delas prestou cada informação.}

\medskip\noindent\textbf{9.2.6\quad Continuidade institucional}\\[2pt]
Nos últimos dez anos, os órgãos e programas que atenderam a comunidade mantiveram atuação contínua?\\
$\bigcirc$~Atuação contínua \quad $\bigcirc$~Atuação intermitente \quad $\bigcirc$~Atuação encerrada \quad $\bigcirc$~Nunca houve atuação\\[2pt]
O acesso a direitos ou programas passou por intermediação política ou eleitoral?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim. Descreva: \dotfill

\medskip\noindent\textbf{9.2.7\quad Infraestrutura coletiva de beneficiamento}\\[2pt]
A comunidade dispõe de infraestrutura coletiva de beneficiamento da produção (casa de farinha, moinho, casa de sementes, trator de uso comum)?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim, em funcionamento \quad $\bigcirc$~Sim, desativada ou em ruína\\[2pt]
Relacione cada equipamento, quem o administra e o estado de funcionamento: \dotfill\\[2pt]
Como se remunera o uso por quem não é proprietário?\\
$\square$~Não há remuneração \quad $\square$~Pagamento em dinheiro \quad $\square$~Retenção de parte do produto beneficiado \quad $\square$~Troca de dias de trabalho \quad $\square$~Outro: \dotfill

\medskip\noindent\textbf{9.2.8\quad Desagregação da resposta por gênero e geração}\\[2pt]
\textit{Item de controle de aplicação, preenchido pelo compilador.}\\[2pt]
Para os campos de priorização de impactos e de urgência de salvaguarda, foram registradas ordenações separadas para mulheres, para pessoas com menos de 30 anos e para mestres de saberes com mais de 60 anos?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim\\[2pt]
Houve divergência entre as ordenações desses estratos?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim. Registre a divergência sem conciliá-la: \dotfill\\[2pt]\dotfill

{Camada~2 --- Inventário etnobotânico e ecológico (agricultor + pesquisador em campo)}
...
%% --- V2 ---
\medskip\noindent\textbf{9.3.4\quad Critério local de seleção e guarda de semente}\\[2pt]
Quem decide, na unidade familiar, quais sementes são guardadas para o plantio seguinte?\\
$\bigcirc$~Homem \quad $\bigcirc$~Mulher \quad $\bigcirc$~Decisão conjunta \quad $\bigcirc$~Pessoa mais velha da casa\\[2pt]
Qual o critério declarado de seleção?\\
$\square$~Resistência à seca \quad $\square$~Ciclo curto \quad $\square$~Sabor ou qualidade culinária \quad $\square$~Produtividade \quad $\square$~Adequação ao tipo de solo \quad $\square$~Outro: \dotfill\\[2pt]
A comunidade mantém variedades de ciclos distintos como forma de reduzir o risco de perda total da safra?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim. Relacione as variedades e o risco que cada uma cobre: \dotfill\\[2pt]\dotfill\\[2pt]
Onde e como as sementes são guardadas entre as safras? \dotfill

\medskip\noindent\textbf{9.3.5\quad Farmacopeia local e uso ritual}\\[2pt]
Relacione as espécies de uso medicinal nomeadas localmente, com a indicação declarada e o modo de preparo:\\[2pt]
\begin{tabular}{p{3.6cm}p{3.6cm}p{3.2cm}p{2.4cm}}
\textit{Nome local} & \textit{Indicação declarada} & \textit{Modo de preparo} & \textit{Uso ritual} \\
\dotfill & \dotfill & \dotfill & $\bigcirc$~Não~$\bigcirc$~Sim \\
\dotfill & \dotfill & \dotfill & $\bigcirc$~Não~$\bigcirc$~Sim \\
\dotfill & \dotfill & \dotfill & $\bigcirc$~Não~$\bigcirc$~Sim \\
\end{tabular}\\[4pt]
Alguma dessas espécies é empregada em benzimento, reza ou outro procedimento ritual?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim. Especifique a espécie e o procedimento: \dotfill\\[2pt]
Quem detém o conhecimento do preparo e a quem ele é transmitido? \dotfill
...
%% ---------------------------------------------------------
\subsubsection*\medskip\noindent\textbf{9.4.4\quad Composição do portfólio familiar de atividades}\\[2pt]
\textit{Este item registra o conjunto de atividades que a unidade familiar opera de modo articulado, e não uma prática isolada.}\\[2pt]
Quantas atividades produtivas distintas a unidade familiar conduz em um mesmo ano agrícola?\\
$\bigcirc$~Uma \quad $\bigcirc$~Duas \quad $\bigcirc$~Três \quad $\bigcirc$~Quatro \quad $\bigcirc$~Cinco ou mais\\[2pt]
Relacione cada atividade, a área ou o efetivo aproximado e a destinação predominante:\\[2pt]
\begin{tabular}{p{4.2cm}p{3.4cm}p{4.8cm}}
\textit{Atividade} & \textit{Área ou efetivo} & \textit{Destinação predominante} \\
\dotfill & \dotfill & \dotfill \\
\dotfill & \dotfill & \dotfill \\
\dotfill & \dotfill & \dotfill \\
\dotfill & \dotfill & \dotfill \\
\end{tabular}\\[4pt]
Qual o critério com que a família distribui trabalho e terra entre essas atividades?\\
$\square$~Calendário de chuvas \quad $\square$~Disponibilidade de mão de obra \quad $\square$~Garantia de alimento na casa \quad $\square$~Expectativa de preço \quad $\square$~Aptidão do solo de cada parcela \quad $\square$~Outro: \dotfill\\[2pt]
Alguma dessas atividades depende das demais para se sustentar?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim. Descreva a dependência: \dotfill
...
%% --- V4 ---
\medskip\noindent\textbf{9.5.4\quad Custódia e suporte do acervo probatório}\\[2pt]
Onde se encontram os documentos que comprovam a história e o reconhecimento da comunidade?\\
$\square$~Sede da associação \quad $\square$~Residência de morador \quad $\square$~Órgão público \quad $\square$~Universidade ou pesquisador \quad $\square$~Não se sabe\\[2pt]
Em que suporte estão registrados?\\
$\square$~Papel \quad $\square$~Arquivo digital em computador \quad $\square$~Mídia removível (pendrive, cartão, CD) \quad $\square$~Aplicativo de mensagens \quad $\square$~Nuvem\\[2pt]
Existe cópia de segurança fora do território? \quad $\bigcirc$~Não \quad $\bigcirc$~Sim \quad $\bigcirc$~Não se sabe\\[2pt]
Quem autoriza o acesso de terceiros a esse acervo? \dotfill

\medskip\noindent\textbf{9.5.5\quad Alteração toponímica e etnônimo suprimido}\\[2pt]
A denominação oficial da localidade difere daquela empregada pelos moradores?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim\\[2pt]
Se sim, informe a denominação local, a denominação oficial e o ano aproximado da mudança:\\[2pt]
Local: \dotfill\quad Oficial: \dotfill\quad Ano: \dotfill\\[2pt]
Quem promoveu a mudança e qual a razão declarada?\\
$\square$~Administração escolar \quad $\square$~Prefeitura \quad $\square$~Agente político \quad $\square$~Órgão de registro \quad $\square$~Outro: \dotfill\\[2pt]
A denominação substituída continha termo que marcava a identidade do grupo?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim. Termo suprimido: \dotfill
...
\subsubsection*\medskip\noindent\textbf{9.6.5\quad Regime de posse interno ao território coletivo}\\[2pt]
Dentro do território de titularidade coletiva, as áreas de uso são divididas entre famílias?\\
$\bigcirc$~Não, o uso é integralmente comum \quad $\bigcirc$~Sim, há parcelas familiares reconhecidas entre os moradores \quad $\bigcirc$~Arranjo misto\\[2pt]
Como se reconhece o limite de cada parcela?\\
$\square$~Marco natural \quad $\square$~Cerca \quad $\square$~Memória compartilhada \quad $\square$~Documento interno \quad $\square$~Outro: \dotfill\\[2pt]
Quais recursos permanecem de uso comum a despeito do parcelamento?\\
$\square$~Pastagem \quad $\square$~Aguada, barreiro ou cacimba \quad $\square$~Área de coleta \quad $\square$~Caminho de acesso \quad $\square$~Outro: \dotfill\\[2pt]
Como se resolve a transmissão da parcela entre gerações? \dotfill

\medskip\noindent\textbf{9.6.6\quad Expropriação da safra e pressão sobre a área cultivada}\\[2pt]
A comunidade registra episódios de perda da safra por ação de terceiros?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim, no passado \quad $\bigcirc$~Sim, ainda hoje\\[2pt]
Se sim, qual o procedimento empregado?\\
$\square$~Introdução de gado na roça antes da colheita \quad $\square$~Ocupação de área cultivada \quad $\square$~Impedimento de acesso à água \quad $\square$~Destruição de cerca ou plantio \quad $\square$~Outro: \dotfill\\[2pt]
Período aproximado e frequência: \dotfill\\[2pt]
Houve alguma providência formal? \quad $\bigcirc$~Não \quad $\bigcirc$~Sim. Qual: \dotfill
...
\medskip\noindent\textbf{9.7.4\quad Repertório musical, coreográfico e instrumental}\\[2pt]
A comunidade mantém repertório musical ou coreográfico próprio, vinculado às celebrações ou ao trabalho?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim, mantido por poucos \quad $\bigcirc$~Sim, praticado coletivamente\\[2pt]
Relacione as manifestações, os instrumentos empregados e quem os executa: \dotfill\\[2pt]\dotfill\\[2pt]
Nas apresentações públicas da comunidade, o repertório executado é o local ou foi substituído por repertório de fora?\\
$\bigcirc$~Integralmente local \quad $\bigcirc$~Predominantemente local \quad $\bigcirc$~Misto \quad $\bigcirc$~Predominantemente de fora\\[2pt]
Se houve substituição, indique o que foi deslocado e desde quando: \dotfill\\[2pt]
Os instrumentos são construídos localmente? \quad $\bigcirc$~Não \quad $\bigcirc$~Sim. Quem os constrói: \dotfill
...
\medskip\noindent\textbf{9.8.3\quad Composição da renda e elos não monetários de circulação}\\[2pt]
Quais fontes compõem a renda da unidade familiar?\\
$\square$~Venda da produção \quad $\square$~Aposentadoria rural \quad $\square$~Transferência assistencial \quad $\square$~Trabalho assalariado fora \quad $\square$~Artesanato \quad $\square$~Outro: \dotfill\\[2pt]
Qual a proporção aproximada do alimento consumido que é produzido na própria unidade?\\
$\bigcirc$~Menos de 25\% \quad $\bigcirc$~25--50\% \quad $\bigcirc$~50--75\% \quad $\bigcirc$~Mais de 75\%\\[2pt]
Parte da produção circula sem passar por dinheiro?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim\\[2pt]
Se sim, assinale as formas praticadas:\\
$\square$~Troca entre famílias \quad $\square$~Pagamento de serviço em produto \quad $\square$~Doação em festa ou mutirão \quad $\square$~Retenção de parte do produto pelo uso de equipamento \quad $\square$~Outro: \dotfill

\medskip\noindent\textbf{9.8.4\quad Beneficiamento de fibra e tecnologias de substituição}\\[2pt]
A comunidade beneficia fibra vegetal ou animal para uso próprio ou venda?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim, para uso próprio \quad $\bigcirc$~Sim, para venda \quad $\bigcirc$~Sim, para ambos\\[2pt]
Relacione a fibra, o produto obtido e o instrumento empregado: \dotfill\\[2pt]\dotfill\\[2pt]
Existem técnicas de preparo de sucedâneos acionadas quando falta energia elétrica ou produto de mercado (por exemplo, torrefação de grão para substituir café)?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim\\[2pt]
Se sim, descreva a técnica, o produto substituído e o período em que a prática vigorou ou vigora: \dotfill\\[2pt]\dotfill
...
\medskip\noindent\textbf{9.9.3\quad Espécies retiradas do portfólio, com dupla referência temporal}\\[2pt]
\textit{Este item registra a situação anterior ao abandono ao lado da situação atual, porque uma única referência temporal captura apenas o estado corrente de um processo em curso.}\\[2pt]
Alguma espécie deixou de ser cultivada na comunidade por restrição hídrica ou climática?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim\\[2pt]
Se sim, preencha para cada espécie:\\[2pt]
\begin{tabular}{p{2.8cm}p{2.6cm}p{2.6cm}p{2.4cm}p{2.4cm}}
\textit{Espécie} & \textit{Área antes} & \textit{Área hoje} & \textit{Ano aproximado da retração} & \textit{Motivo declarado} \\
\dotfill & \dotfill & \dotfill & \dotfill & \dotfill \\
\dotfill & \dotfill & \dotfill & \dotfill & \dotfill \\
\dotfill & \dotfill & \dotfill & \dotfill & \dotfill \\
\end{tabular}\\[4pt]
Alguma dessas espécies ainda é cultivada em parcela reduzida ou apenas para semente?\\
$\bigcirc$~Não \quad $\bigcirc$~Sim. Especifique: \dotfill\\[2pt]
Houve substituição por outra espécie? \quad $\bigcirc$~Não \quad $\bigcirc$~Sim. Qual: \dotfill
